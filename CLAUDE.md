...
- **2026-07-22** — **Merge-readiness Phase 3 (partial): Team & Access, per-tenant credentials, payments, cron wiring, forecasting** (same branch `feature/phase1-multitenancy-auth`). Builds on Phases 1–2. **No schema changes** — everything reuses existing columns (`Tenant.members`/`credentials`/`phoneNumber`, `Payment`, `Ticket.payment`), so **no `db push`/migration is needed**. `npm run build` clean (37 routes).
  ...
  - **NOT built (remaining Phase 3):** **3B** deterministic invoice-template parser + Gemini fallback — deferred: needs the user's real recurring-vendor sample PDFs to seed templates. **3E** analytics inbound/outbound segmentation page — deferred (the outbound dashboard + forecasting already cover the live-data analytics need; a dedicated Analytics tab with tag segmentation is the remaining piece).
