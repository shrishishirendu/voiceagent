...
- **2026-07-22** — **Merge-readiness Phase 1: multi-tenancy + auth + tickets + customer_id** (branch `feature/phase1-multitenancy-auth`, pushed to `origin` — commit `2359f36`; open a PR at github.com/shrishishirendu/voiceagent). First phase of evolving this app to adopt EnvoyIn's patterns so it can later be merged INTO EnvoyIn (plan: `~/.claude/plans/i-am-working-on-modular-tiger.md`). This is the foundation that unblocks Phases 2–3.
...
- **2026-07-22** — **Merge-readiness Phase 2: Customers CRM + Outbound dashboard + Onboarding** (same branch `feature/phase1-multitenancy-auth`, pushed to `origin` — commit `c6f1b3b`). Builds on Phase 1's tenancy/tickets foundation.
