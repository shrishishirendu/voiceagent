...
- **2026-07-22** — **Merge-readiness Phase 1: multi-tenancy + auth + tickets + customer_id** (branch `feature/phase1-multitenancy-auth`). First phase of evolving this app to adopt EnvoyIn's patterns so it can later be merged INTO EnvoyIn (plan: `~/.claude/plans/i-am-working-on-modular-tiger.md`). This is the foundation that unblocks Phases 2–3.
